...
normal_count = 0
miss_detection = open("MissDetection.txt", "w")
normal_result = {"anomaly_score":[], "w1_score":[], "w2_score":[]}

for i,file in enumerate(normal_list):
    anomaly_score = []
    w1_score = []
    w2_score = []

    df_normal = pd.read_csv(normal_list[i], index_col=0)
    window_anomaly = slice_data(df_normal, window).reshape(-1, 6 * window)
    ...
    if score > 0.04:
        ...
        miss_detection.write(file+"\n")

    normal_result["anomaly_score"].append(score)
    normal_result["w1_score"].append(avg_w1)
    normal_result["w2_score"].append(avg_w2)

print("{}/{}".format(normal_count, len(normal_list)))
result = pd.DataFrame.from_dict(normal_result)
result.to_csv("inference_normal.csv")
